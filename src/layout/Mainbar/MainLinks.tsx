...
const MainLink = ({
  ...
  return (
    <NavigationMenu.Item
      className={clsx(
        `${borderColor} sm:px-5 py-2 sm:border-[1px] sm:border-FFFFF sm:border-opacity-20`,
        {
...
const MainLinks: React.FC<MainLinksProps> = ({ planetData }) => {
  return (
    <NavigationMenu.Root>
      <NavigationMenu.List className="flex justify-between px-6 sm:flex-col sm:gap-4">
        {links.map((item, id) => (
          <MainLink
            href={item.href}
            key={item.name}
